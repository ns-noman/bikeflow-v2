...
class BikeServiceRecordController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $bikeServiceRecord->account_id = $account_id;
    ...
    public function list(Request $request)
    {
        ...
        $totalBikeServiceRecordAmount = BikeServiceRecord::where('status', 1)->sum('total_amount');

        $query = BikeServiceRecord::join('bike_purchases', 'bike_purchases.id', '=', 'bike_service_records.bike_purchase_id')
                            ->join('bikes', 'bikes.id', '=', 'bike_purchases.bike_id')
                            ->join('bike_models', 'bike_models.id', '=', 'bikes.model_id')
                            ->join('colors', 'colors.id', '=', 'bikes.color_id')
                            ->join('accounts', 'accounts.id', '=', 'bike_service_records.account_id')
        ...
        if(!$request->has('order')){
            $query = $query->orderBy('bike_service_records.status','asc')
                    ->orderBy('bike_service_records.id','desc');
        }
        $query = $query->select($select);
        return DataTables::of($query)->with(['totalBikeServiceRecordAmount'=> $totalBikeServiceRecordAmount])->make(true);
    }
    ...
    public function approve($id)
    {
        ...
        $this->accountTransaction([
            'account_id'        => $bikeServiceRecord->account_id,
            'debit_amount'      => $bikeServiceRecord->total_amount,
            'reference_number'  => $bikeServiceRecord->reference_number,
            'description'       => 'Bike Service Expense',
            'transaction_date'  => $bikeServiceRecord->date,
        ]);
        $this->investorLedger([
            'investor_id'       => 1,
            'account_id'        => $bikeServiceRecord->account_id,
            'debit_amount'      => $bikeServiceRecord->total_amount,
            'transaction_date'  => $bikeServiceRecord->date,
            'particular'       => "Bike Service Expense",
        ]);
